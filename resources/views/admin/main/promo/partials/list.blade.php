@php
    $isStandalone = request()->routeIs('admin.main.promo.*');
    $actionRoute = $isStandalone ? route('admin.main.index') : route('admin.main.promo.index');
    $actionLabel = $isStandalone ? 'Kembali ke Dashboard' : 'Kelola Promo';
    $headerTitle = $isStandalone ? 'Kelola Promo' : 'Preview Promo ';
@endphp
...
<div class="space-y-6">
    ...
    <div class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
                        ...
                        <tr class="hover:bg-slate-50 transition">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex justify-end gap-2">
                                    ...
                                    {{-- DELETE --}}
                                    <form id="delete-form-promo-{{ $promo->id }}"
                                        action="{{ route('admin.main.promo.destroy', $promo->id) }}"
                                        method="POST"
                                        class="inline">
                                        @csrf
                                        @method('DELETE')
        ...
        @if($isStandalone && method_exists($promos, 'links'))
...
<script>
    function confirmDeletePromo(id, title) {
        Swal.fire({
            title: 'Hapus Promo?',
            html: `Apakah Anda yakin ingin menghapus promo <br><strong>"${title}"</strong>?`,
            icon: 'warning',
