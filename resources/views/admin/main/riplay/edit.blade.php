...
@section('title', 'Ubah Dokumen Riplay')

@section('content')
<div class="max-w-3xl mx-auto space-y-6">

    <div>
        <h1 class="text-2xl font-semibold text-slate-800">Ubah Dokumen Riplay</h1>
        <p class="text-sm text-slate-500 mt-1">Perbarui detail dokumen dan opsi visibilitas.</p>
    </div>

    <form action="{{ route('admin.riplay.update', $riplay) }}"
          method="POST"
        ...
        <div class="space-y-1.5">
            ...
            <select name="type"
                    required
                    class="w-full rounded-xl bg-slate-50 border border-slate-200 px-4 py-2.5 text-slate-700 focus:bg-white focus:border-[#00326B] focus:ring-4 focus:ring-[#00326B]/10 transition">
                <option value="" disabled {{ old('type', $riplay->type) ? '' : 'selected' }}>-- Pilih Tipe --</option>
                <option value="kredit" {{ old('type', $riplay->type) === 'kredit' ? 'selected' : '' }}>Kredit</option>
                <option value="deposito" {{ old('type', $riplay->type) === 'deposito' ? 'selected' : '' }}>Deposito</option>
                <option value="tabungan" {{ old('type', $riplay->type) === 'tabungan' ? 'selected' : '' }}>Tabungan</option>
            </select>
        </div>
        @error('type')
            <p class="text-xs text-red-600">{{ $message }}</p>
        ...
        <div class="space-y-1.5">
            ...
            <textarea name="description"
                      ...
                      class="w-full rounded-xl bg-slate-50 border border-slate-200 px-4 py-3 text-slate-700 focus:bg-white focus:border-[#00326B] focus:ring-4 focus:ring-[#00326B]/10 transition resize-none">{{ old('description', $riplay->description) }}</textarea>
        </div>
        @error('description')
            <p class="text-xs text-red-600">{{ $message }}</p>
        @enderror
        ...
        <div class="space-y-1.5">
            <label class="text-sm font-medium text-slate-600">Ganti File PDF</label>
            <input type="file"
                   name="document"
            ...
            <p class="text-xs text-slate-500">Biarkan kosong jika tidak ingin mengganti file.</p>
            @if($riplay->file_path)
                <p class="text-xs text-slate-500">
                    File saat ini:
                    <a href="{{ asset('storage/' . $riplay->file_path) }}" target="_blank" class="font-medium text-[#00326B] underline">
                        {{ basename($riplay->file_path) }}
                    </a>
                </p>
            @endif
        </div>
        ...
        <div class="flex items-center gap-2">
            ...
            <label class="inline-flex items-center gap-2 text-sm font-medium text-slate-600">
                <input type="checkbox"
                       name="is_active"
                       value="1"
                       {{ old('is_active', $riplay->is_active ? '1' : '0') == '1' ? 'checked' : '' }}
